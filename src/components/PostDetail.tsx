...
import { Heart, MessageCircle, Share, ChevronLeft, Search, SmilePlus, X, Sparkles, PlayCircle, Download, Upload, Image as ImageIcon, RotateCcw, Send, Edit, Star } from 'lucide-react';
...
// 小团组视觉资产 - 从配置文件加载
let cachedClusterAssets: any[] = [];

// 存储每个小团组ID对应的视觉资产
let clusterAssetCache: Record<string, any> = {};

// 加载小团组视觉资产配置
const loadClusterAssetsFromConfig = async () => {
  try {
    const response = await fetch('/cluster-assets/config.json');
    const data = await response.json();
    cachedClusterAssets = data.clusters || [];
    console.log(`✅ 加载了 ${cachedClusterAssets.length} 个小团组视觉资产`);
    return cachedClusterAssets;
  } catch (error) {
    console.error('❌ 加载小团组视觉配置失败:', error);
    return [];
  }
};

// 清除小团组视觉资产缓存
const clearClusterAssetCache = () => {
  clusterAssetCache = {};
};

// 随机选择小团组视觉资产，根据ID保持一致
const getRandomClusterAssetForId = (id: string) => {
  if (!clusterAssetCache[id]) {
    if (cachedClusterAssets.length > 0) {
      clusterAssetCache[id] = cachedClusterAssets[Math.floor(Math.random() * cachedClusterAssets.length)];
    } else {
      clusterAssetCache[id] = {
        name: "默认光晕",
...
export default function PostDetail({ onBack, refreshGifs, refreshClusters, clusterAuraEnabled = true, clusterParticlesEnabled = true, clusterLocalAssetsEnabled = true, customInteractionEffects = {}, customInteractionGifs = {} }: { onBack: () => void; refreshGifs?: number; refreshClusters?: number; clusterAuraEnabled?: boolean; clusterParticlesEnabled?: boolean; clusterLocalAssetsEnabled?: boolean; customInteractionEffects?: Record<string, string>; customInteractionGifs?: Record<string, string> }) {
  ...
  useEffect(() => {
    const reloadConfigs = async () => {
      ...
      await loadClusterAssetsFromConfig();
      initializeImages();
    };
    reloadConfigs();
  }, []);
  ...
  // 刷新小团组视觉时清除缓存并重新加载配置
  useEffect(() => {
    const reloadClusterAssets = async () => {
      if (refreshClusters && refreshClusters > 0) {
        clearClusterAssetCache();
        await loadClusterAssetsFromConfig();
        console.log('🧹 小团组视觉缓存已清除，重新加载配置');
      }
    };
    reloadClusterAssets();
  }, [refreshClusters]);
  ...
  const [replayData, setReplayData] = useState<{sourceIp: string, targetIp: string, action: any} | null>(null);
  ...
  // 剧场模式专属视觉资产
  const [theaterVisualAsset, setTheaterVisualAsset] = useState<string | null>(null);
  ...
  const handleReply = (targetId: string, isGroup: boolean = false, customGif?: string) => {
    console.log('handleReply called with:', { targetId, isGroup, customGif });
    setReplyingTo(targetId);
    setReplyingToGroup(isGroup);
    setInputFocused(true);
    if (customGif) {
      setCustomUserIp(customGif);
    }
    console.log('After setting state, inputFocused should be true');
  };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && uploadingImageKey) {
      ...
      // 如果是剧场模式的视觉资产
      if (uploadingImageKey === 'theater_visual_asset') {
        setTheaterVisualAsset(compressed);
      } else {
        // 否则是普通的 IP 图片替换
        setCustomImages(prev => {
          const newImages = { ...prev, [uploadingImageKey]: compressed };
          
          // Prevent QuotaExceededError by keeping the size of customImages under ~4MB
          let stringified = JSON.stringify(newImages);
          while (stringified.length > 4 * 1024 * 1024) {
            const keys = Object.keys(newImages);
            if (keys.length <= 1) break;
            // Remove the oldest key (assuming insertion order) that is not the one we just added
            const keyToRemove = keys.find(k => k !== uploadingImageKey);
            if (keyToRemove) {
              delete newImages[keyToRemove];
              stringified = JSON.stringify(newImages);
            } else {
              break;
            }
          }
  ...
  return (
    <>
      <div className="w-full h-full bg-transparent overflow-hidden relative flex flex-col max-w-full">
        ...
        <div ref={scrollContainerRef} className={`flex-1 overflow-y-auto flex flex-col ${viewMode === 'list' ? 'pb-32 bg-white' : 'pb-0 bg-transparent'}`} onClick={handleBgClick}>
          ...
          {viewMode === 'list' ? (
            ...
            <OpenWorldView 
              ...
              theaterVisualAsset={theaterVisualAsset}
              setTheaterVisualAsset={setTheaterVisualAsset}
              inputText={inputText}
        ...
        {/* Bottom Input Area */}
        <div className="absolute bottom-0 left-0 w-full max-w-full bg-white border-t border-[#f5f5f5] z-[200]">
          
          <AnimatePresence>
            {inputFocused && showRecommendations && (
              <motion.div 
                initial={{ y: '100%', opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                exit={{ y: '100%', opacity: 0 }}
                transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                className="absolute bottom-full left-0 w-full bg-white/40 backdrop-blur-2xl border-t border-white/40 p-3 shadow-[0_-10px_30px_rgba(0,0,0,0.05)]"
              >
                {isAnalyzing ? (
                  <div className="flex items-center gap-2 px-4 py-3 text-sm text-[#333333] font-medium">
                  ...
                  <>
                    <div className="flex items-center gap-3 mb-3 px-2">
                      <div className="flex flex-col">
                        <span className="text-sm text-[#0B2B26] font-medium">
                          {aiPrompt}
                        </span>
                      </div>
                    </div>
                    
                    <div className="flex gap-2 overflow-x-auto pb-1 hide-scrollbar px-2">
                      {IMAGES_LIST.map((img, idx) => {
                        const emojiKey = `emoji_${idx}`;
                        const isSelected = customUserIp === (customImages[emojiKey] || img);
                        return (
                        <button 
                          key={idx}
                          onClick={() => setCustomUserIp(customImages[emojiKey] || img)}
                          onContextMenu={(e) => { e.preventDefault(); handleImageContextMenu(e, emojiKey); }}
                          className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 transition-all focus:outline-none focus:ring-0 active:outline-none active:ring-0 ${isSelected ? 'bg-white/60 backdrop-blur-md border border-white/80 shadow-[0_0_12px_rgba(255,255,255,0.8)]' : 'bg-white/20 backdrop-blur-sm border border-white/30 hover:bg-white/40'}`}
                          title="右键替换表情"
                        >
                          <SafeIpImage src={customImages[emojiKey] || img} className="w-8 h-8 object-contain" alt="Emoji" />
                        </button>
                      )})}
                    </div>
          ...
          {viewMode === 'world' ? (
            <>
              {/* 世界视图下的评论输入框 - 胶囊状悬浮在最上层 */}
              {!inputFocused && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[250]">
                  <div className="flex items-center gap-1.5 bg-white/95 backdrop-blur-xl rounded-full px-3 py-1.5 shadow-2xl border border-white/50">
                    ...
                    {/* 输入框 */}
                    <div className="bg-gray-100 rounded-full h-7 flex items-center px-2.5 min-w-[140px]">
                      <input 
                        type="text" 
                        value={inputText}
                        onChange={(e) => setInputText(e.target.value)}
                        placeholder={replyingTo ? "回复..." : "说点什么..."}
                        className="bg-transparent flex-1 text-xs outline-none text-gray-700 placeholder-gray-400"
                        onFocus={() => {
                          if (!replyingTo) setReplyingTo('post');
                          setInputFocused(true);
                        }}
                      />
                    </div>
                    ...
                    {inputFocused ? (
                      <button 
                        ...
                        className={`w-7 h-7 rounded-full flex items-center justify-center font-medium transition-all bg-gray-100 hover:bg-gray-200 ${
                          !isAnalyzing
                            ? 'text-gray-700' 
                      ...
                      <div className="flex items-center gap-1.5 text-gray-600">
                        <div className="flex items-center gap-0.5">
                          ...
                          <span className="text-[11px] font-normal">870</span>
                        </div>
                        <div className="flex items-center gap-0.5">
                          <Star className="w-3.5 h-3.5" />
                          <span className="text-[11px] font-normal">351</span>
                        </div>
                        <div className="flex items-center gap-0.5">
                          <MessageCircle className="w-3.5 h-3.5" />
                          <span className="text-[11px] font-normal">36</span>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <button 
                ...
                className="fixed bottom-8 right-8 z-[100] w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center hover:bg-gray-50 transition-colors"
                title="上传 GIF"
              >
            ...
            /* 常规评论视图下的输入框 - 保持原有样式 */
            <div className="flex items-center gap-3 bg-white relative z-10 px-4 py-4">
              <div className="flex-1 bg-[#f5f5f5] rounded-full h-7 flex items-center px-3 gap-2">
                <Edit className="w-3 h-3 text-[#999999]" />
                <input 
                  type="text" 
                  value={inputText}
                  onChange={(e) => setInputText(e.target.value)}
...
const OpenWorldView = forwardRef(({ comments, customUserIp, interactionCounts, setInteractionCounts, lastInteractions, setLastInteractions, handleReplay, handleGroupReplay, handleReply, customMoods, ipCustomActions, initialActions, customImages, handleImageContextMenu, customBgImage, handleBgContextMenu, handleResetBg, handleActionContextMenu, handleAddReplyFromAction, isTabsSticky, inputFocused = false, clusterAuraEnabled = true, clusterParticlesEnabled = true, clusterLocalAssetsEnabled = true, customInteractionEffects = {}, customInteractionGifs = {}, setViewMode, theaterVisualAsset, setTheaterVisualAsset, inputText, setInputText, setInputFocused, refreshClusters }: any, ref: any) => {
    ...
    return (
      ...
      >
        <motion.div
          ...
          transition={{ 
            duration: item.isNew ? 0.5 : (isInteracting ? 0.5 : 0.2),
            ease: "easeInOut"
          }}
        >
          {/* 气泡评论 */}
          {item.text && (
            <div 
              className={`absolute bottom-full mb-2 w-max max-w-[160px] bg-white rounded-lg p-2.5 shadow-lg transition-opacity duration-500 pointer-events-auto flex flex-col items-start z-[120] ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} 
              style={{ 
            ...
            >
              ...
              <div className="text-xs text-[#333333] leading-tight text-left relative pr-0">
          ...
          <div 
            className={`relative cursor-pointer mt-1 flex items-center justify-center`}
            style={{
              width: isParent ? '80px' : '72px',
              height: isParent ? '80px' : '72px',
              transform: `scale(${1/scale})`, // 反向缩放，保持视觉大小不变
              transformOrigin: 'center center'
            }}
            onPointerDown={(e) => onPointerDown(e, item, count)}
            onPointerUp={(e) => onPointerUp(e, item, count)}
            onClick={(e) => onClick(e, item, count)}
          ...
          >
            <SafeIpImage 
              src={randomGif} 
              className="w-full h-full object-contain transition-all duration-300 group-hover:scale-110 pointer-events-none relative z-10" 
              style={{ filter: stateStyle.filter }}
              alt="IP" 
            />
          </div>
  ...
  <>
    <div 
      ref={containerRef} 
      className="flex items-center justify-center bg-transparent relative z-10 w-full min-h-[750px] shrink-0 overflow-hidden p-0"
      onWheel={(e) => {
        e.preventDefault();
        e.stopPropagation();
      }}
      ...
      >
        <div className="absolute top-7 right-7 z-50">
          <button
            ...
            className="w-10 h-10 bg-white/40 backdrop-blur-2xl rounded-full shadow-lg flex items-center justify-center text-gray-700 hover:text-gray-900 transition-colors border border-white/40"
            title="背景设置"
          >
            <ImageIcon size={20} />
          </button>
          ...
          <AnimatePresence>
            {bgMenuOpen && (
              <motion.div
                ...
                className="absolute top-full right-0 mt-2 w-32 bg-white/50 backdrop-blur-2xl rounded-xl shadow-xl border border-white/40 overflow-hidden flex flex-col"
              >
                <button
                  onClick={() => {
                    handleBgContextMenu();
                  ...
                  className="px-4 py-3 text-sm text-left hover:bg-black/5 transition-colors flex items-center gap-2 text-gray-700"
                >
                  <Upload size={14} />
                  更换背景
                </button>
                {customBgImage && (
                  <button
                    ...
                    className="px-4 py-3 text-sm text-left hover:bg-black/5 transition-colors text-gray-700 flex items-center gap-2 border-t border-black/5"
                  >
                    <RotateCcw size={14} />
                    恢复默认
                  </button>
      ...
      >
                ...
                {isImageAsset ? (
                  ...
                  <div 
                    ...
                    style={{
                      ...
                      filter: `blur(${clusterAsset.blur || 12}px)`,
                      opacity: clusterAsset.opacity || 0.6
                    }}
                  />
                )}
              </div>
        ...
        <div
          ...
          style={{ 
            ...
            opacity: theaterMode && parent.id !== theaterMode ? 0 : 1
          }}
        >
          <div
            className="relative flex items-center justify-center"
          >
            {/* Invisible clickable area for the cluster - 高 z-index 确保点击不被阻挡 */}
            <div
              className="absolute cursor-pointer z-[150] rounded-full bg-transparent"
              style={{
                width: `${100 + (parent.children.length > 4 ? 20 : 0)}px`,
                height: `${60 + (parent.children.length > 4 ? 10 : 0)}px`,
              }}
              onClick={(e) => {
                e.stopPropagation();
                handleCloudClick(parent);
              }}
            />
    ...
    {theaterMode && (
      <>
          ...
          if (parent.id === theaterMode) {
            return (
              <motion.div
                ...
                transition={{ duration: 0.5 }}
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[210] w-full max-w-[400px] px-4"
              >
                {/* 评论内容显示 - 白字 + 头像，随机分布 */}
                <div className="relative h-[300px] mb-4">
                  ...
                  {parent.children.map((child: any, index: number) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-white text-xs leading-tight opacity-80">
                          {child.text}
                        </div>
                      </motion.div>
                    );
                ...
                <div className="relative flex items-center justify-center py-4">
                  <div 
                    ...
                    onContextMenu={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      handleImageContextMenu(e, 'theater_visual_asset');
                    }}
                    title="右键替换此视觉资产"
                  >
                    <SafeIpImage 
                      src={theaterVisualAsset || getRandomGifForId('theater_default')} 
                      className="w-full h-full object-contain transition-all duration-300 hover:scale-110 pointer-events-none relative z-10"
                      alt="剧场视觉资产" 
                    />
                  </div>
                </div>
        ...
        >
          <button
            onClick={() => setTheaterMode(null)}
            className="w-10 h-10 bg-white/80 backdrop-blur-xl rounded-full shadow-lg flex items-center justify-center text-gray-700 hover:text-gray-900 transition-colors border border-white/40"
          >
        ...
        {/* 剧场模式下显示弧形选择框 */}
        {!inputFocused && (
          <div className="absolute bottom-0 left-0 right-0 z-[215] pointer-events-none" style={{ transform: 'translateY(80px)' }}>
            {/* 弧形轨道背景 - 使用 flex 居中 */}
            <div className="relative w-full h-[200px] overflow-visible pointer-events-auto flex justify-center">
              {/* 单一背景层 - F3F3F1 和 FDF2E7 弥散渐变 + 明显阴影 */}
              <div 
              ...
              {/* 可滑动的 IP 容器 */}
              <div className="absolute bottom-0 left-0 right-0 h-full flex items-end justify-center overflow-hidden z-10" style={{ transform: 'translateY(-80px)' }}>
                <motion.div
                  className="flex items-end gap-2 px-4 pb-8"
                  drag="x"
                  dragConstraints={{ 
                    left: -(bottomIpIds.length * 60 - 300), 
                    right: 0 
                  }}
                  dragElastic={0.05}
                  dragTransition={{ bounceStiffness: 300, bounceDamping: 20 }}
                  style={{ x: slideX, cursor: 'grab' }}
                  whileTap={{ cursor: 'grabbing' }}
                  onDrag={(_, info) => {
                    setSlideX(info.offset.x);
                  }}
                >
                  {bottomIpIds.map((ipId, index) => {
                    ...
                    return (
                      <motion.div
                        key={ipId}
                        className="relative flex flex-col items-center cursor-grab active:cursor-grabbing touch-none flex-shrink-0"
                      ...
                      >
                        <div className="relative">
                          <img 
                            src={gif} 
                            alt="" 
                            className="w-18 h-18 object-contain drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]"
